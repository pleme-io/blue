...
| `mokuroku` | 目録 | catalogue — what every bidama on a BLUE_PATH provides: manifests, glosses, defs with their docs, tests; plus the flat-namespace collision gate | 2026-09-23 | KEEP | Clean with controls: 0 content hits, 0 repo dirs, 0 theory files (control `shikumi` = 1465 files, `caixa` = 138 dirs). **`sakuin` (索引, index) rejected**: a live repo directory and 2 org.yaml rows. The gloss "catalogue" is unclaimed here and in `theory/VOCABULARY.md` (the word appears there only as prose, e.g. CATALOG REFLECTION, which this package is an instance of). |
| `sabi` | 錆 | rust — Rust source from blue data: items as a tree, one renderer (escaping, indentation, identifier checks) | 2026-09-23 | KEEP_WITH_NOTE | The plain noun for the subject (錆 is literally "rust"). 0 repo dirs, 0 theory files, 0 keyword or org.yaml rows (control `shikumi` = 1466 files, `caixa` = 138 dirs); the only content hits are vendored charset test data (not claims) and `Sabiá` in `pleme-brazilian-utils` cities, a place name and Brazil's national songbird. Near in sound, not a false friend for a package of the public language (unlike `mogi`, whose rejection was about a Brazil-first product surface). |
| `tokumei` | 匿名 | anonymity — learn what a group is without exposing anyone: forced-response randomized response and its epsilon, Laplace noise, private counts and shares, small-count suppression, uniqueness risk in small groups | 2026-09-23 | KEEP | Clean with controls: `tokumei` 0 content hits, 0 repo dirs, 0 theory files (control `shikumi` = 1466 files, `caixa` = 138 dirs); `匿名` appears only in vendored locale files (awx `django.po`, faker `lorem.yml`), not claims. The gloss "anonymity" is unclaimed here and in `theory/VOCABULARY.md`. First consumer: the private makoto lab's experience games. |
| `kueri` | クエリ | queries — analysis authored in blue, SQL rendered as the bridge to DuckDB: a query IR, `:core`/`:duckdb` dialect layers, posture rungs, one renderer, ref→DAG | 2026-09-23 | KEEP_WITH_NOTE | Katakana loanword, the plain word a Japanese engineer uses for a database query (SQLクエリ), written honestly like `deeta` and `shisutemu`. Clean with controls (`shikumi` = 1466 files, `caixa` = 138 dirs): 0 repo dirs, 0 theory files, 0 `keywords.tlisp` or `org.yaml` rows, 0 hits in blue or makoto; the one content hit is Adminer's vendored Indonesian locale (`adminer/lang/id.inc.php`), where *kueri* is Indonesian for "query" — the same gloss, not a claim. **`toiawase` (問い合わせ, the Q of 構造化問い合わせ言語 = SQL) was the first choice and is REJECTED:** it contains `awase`, a live pleme-io repo, so every search for that primitive would also return this package (the `kika`→`kikai` direction); and 問い合わせ reads first as a customer inquiry — its four vendored hits are all contact-form strings. Near-homophones `kura`, `kurage`, `kurayami` and sibling `kumiawase` share only `ku`. Adjacent SENSES, recorded rather than merged: `dialeto` (`theory/DIALETO.md`, design-tier relational sub-IR, gloss "dialect" — here dialects are a feature, not the name); `sql-synthesizer` (Rust DDL AST, no SELECT); makoto's private `bunseki` (analysis — it RUNS SQL; kueri BUILDS it, and bunseki's `run_sql` duplicates `q_duckdb`). Every definition carries the `q_` prefix. |
| `shomei` | 署名 | signatures and hash chains — Ed25519 sign/verify, BLAKE3 hashes, tamper-evident chains, over the runtime's pure `crypto` layer | 2026-09-24 | KEEP_WITH_NOTE | The plain noun for the subject (署名 = signature, as in 電子署名). Clean with controls (`shikumi` = 1467 files, `caixa` = 138 dirs): `shomei` 0 content hits, 0 repo dirs, 0 theory files, 0 `keywords.tlisp` or `org.yaml` rows; near-homophone `shoumei` (証明, proof) 0 and 証明 0; `署名` appears once, in vendored `ansible-ui` Japanese locale strings (the same gloss, not a claim). **Sibling to `angou` by contrast:** angou (暗号, cipher) is the classical curriculum and says none of it is secure; shomei encrypts nothing, so a cipher word would teach a falsehood. **Near-homophone inside the distribution:** `seimei` (生命, cellular automata) shares the `-mei` ending; the first mora differs (sho/sei) and neither is a substring of the other, so search separates them. **Adjacent SENSE, recorded rather than merged:** `tameshi` (Rust attestation library: layer types, 3-leaf Merkle, Ed25519, HeartbeatChain) and the `OutcomeReceipt` chains in `theory/CONTINUOUS-SOLUTION-MACHINE.md` chose the same pair (BLAKE3 + Ed25519) independently; shomei is the blue primitive surface, not an attestation model. `sign` is kazu's, so every definition names its role (`sign_message`, `hash_message`, `chain_*`). |
...
| `raifusaikuru` | ライフサイクル | lifecycles, event-sourced — a definition's states, events, fields and guards; `lc_state_of` folds an entity's log into its state, `lc_allowed` names the refusing rule and the task spec, `lc_permit_for` derives a permit's not-after and remaining counts | 2026-09-24 | KEEP_WITH_NOTE | Katakana loanword, the plain word a Japanese engineer uses for a lifecycle (ライフサイクル管理), written honestly like `kueri`, `deeta` and `shisutemu`; long, and every definition carries the short `lc_` prefix (0 `^def lc_` in blue or makoto before this package). Clean with controls (`shikumi` = 1467 files, `caixa` = 138 dirs): `raifusaikuru` and `ライフサイクル` 0 content hits, 0 repo dirs, 0 theory files, 0 `keywords.tlisp` or `org.yaml` rows. Near-homophones `raifu` (4 hits) and `saikuru` (4 hits) are all base64 inside vendored `*.caixa.lisp` files or vendored Unicode name tables, not claims. **The gloss is the dense half**: "lifecycle" names `lifecycle-forge` (FinOps TTL policies), `lifecycle-go` (service start/stop), `breathe-lifecycle`, and three `theory/*-LIFECYCLE.md` files, so the gloss is pinned to the event-sourced engine rather than the bare word, as `deeta` pinned "data" to JSON. **Adjacent SENSE, recorded rather than merged:** `utsuroi` (移ろい, `theory/UTSUROI.md`, crate `utsuroi-lifecycle` in ensaio) is the ephemeral-resource lifecycle *spine*, a Rust stage projection (spawn → ready → keep → drain → reaped) existing domain FSMs map onto; it declares no lifecycles and folds no events, and its named-M1 `(deflifecycle …)` keyword stays its own. `meguri` (巡り) is reserved in utsuroi's family and untouched. First consumer: a private project's lifecycles, built on top. |
...
4. Raise the package-count floor in `bidamas/flake.nix`, the `pkgs.len()` and
   `total` test floors in `distribution.rs`, and regenerate `CATALOG.md`
   (`nix run .#regen`).
